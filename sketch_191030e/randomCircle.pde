  ...
  while (circles.size() < menu_count) {
    ...
    for (PVector p : circles) {
      if (percent[i] >= 0.5) {
        ...
        //円が画面外に表示されない
        if (dist(c.x, c.y, 400, 400) > 245) {
          distlapping = true;
          break;
        }
        //中心に重ならない
        if (dist(c.x, c.y, 400, 400) < (c.z + 65)) {
          centerlapping = true;
          break;
        }
      } else if (percent[i] < 0.5) {
        ...
        //円が画面外に表示されない
        if (dist(c.x, c.y, 400, 400) > 365) {
          distlapping = true;
          break;
        }
        //中心に重ならない
        if (dist(c.x, c.y, 400, 400) < (c.z + 60)) {
          centerlapping = true;
          break;
        }
        if (dist(c.x, c.y, 400, 400) < 270) {
          distinction = true;
          break;
        }
      }
