...
void percentResults() {
  for (int i=0; i<menu_count; i++) {
    for (int j=0; j<menu_count; j++) {
      ...
      if (percent[i] >= 0.5 && percent[j] >= 0.5){
        PercentResultC[i][j] = 11;
        PercentResultW[i][j] = 1.5;
      } else if (percent[i] >= 0.5 && percent[j] < 0.5){
        PercentResultC[i][j] = 10;
        PercentResultW[i][j] = 0.6;
      } else if (percent[i] < 0.5 && percent[j] >= 0.5){
        PercentResultC[i][j] = 10;
        PercentResultW[i][j] = 0.6;
      } else if (percent[i] < 0.5 && percent[j] < 0.5){
        PercentResultC[i][j] = 9;
        PercentResultW[i][j] = 0.3;
      }
    }
  }
}
